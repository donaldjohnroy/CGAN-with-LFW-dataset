...
def get_labels_and_files(folder, number=0):
    ...
    for label in range(0, len(subdir)):
        ...
        filelist.sort()
        # print('this is the filelist: ', len(filelist))

    print('this is the filelist: ', len(filelists))
    ...
    for label in range(0, len(subdir)):
        ...
        print('this is the number of files inside dir: ', len(filelists[label]))
        if len(filelists[label]) is 1:
            filelist=random.sample(filelists[label], 1)
        else:
